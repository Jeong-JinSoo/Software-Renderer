...
void Rasterizer::RasterizerTriangle(const Triangle& tri, std::vector<uint32_t>& frameBuffer, std::vector<float>& depthBuffer, int screenWidth, int screenHeight /*const Texture& texture*/)
{
    ...
    for (int y = bb.minY; y <= bb.maxY; ++y)
    {
        for (int x = bb.minX ; x <= bb.maxX; x++)
        {
            ...
            // uv 보간 (퍼스펙티브 보정)
            float u = ( alpha * tri.v[0].u * tri.v[0].wInv
                      + beta * tri.v[1].u * tri.v[1].wInv
                      + gamma * tri.v[2].u * tri.v[2].wInv) * z;

			float v = ( alpha * tri.v[0].v * tri.v[0].wInv
				      + beta * tri.v[1].v * tri.v[1].wInv
				      + gamma * tri.v[2].v * tri.v[2].wInv) * z;
...
void Rasterizer::RasterizerLine(int screenWidth, int screenHeight, const Point& v0, const Point& v1, std::vector<uint32_t>& frameBuffer, std::vector<float>& depthBuffer, uint32_t color)
{
	...
	for (int i = 0; ; ++i)
	{
		float t = (total == 0) ? 0.0f : (float)i / total;
		// 보간 (z/w, 색 등)
		float iz = z0 * (1 - t) + z1 * t;
		float iw = w0 * (1 - t) + w1 * t;
